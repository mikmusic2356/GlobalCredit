...
import { BATCH_5_ARTICLES } from '../../data/articles/batch5Articles';

export async function seedBatch5Articles() {
  console.log(`[Seed Batch 5] Ingesting ${BATCH_5_ARTICLES.length} US News & Trends articles into Turso DB...`);
  const client = getTursoClient();

  for (const art of BATCH_5_ARTICLES) {
  ...
  console.log(`[Seed Batch 5] Successfully synced ${BATCH_5_ARTICLES.length} News & Trends articles to Turso database!`);
}

if (process.argv[1] && process.argv[1].includes('seedBatch5')) {
  seedBatch5Articles()
    .then(() => {
      console.log('[Seed Batch 5 CLI] Done!');
      process.exit(0);
    })
    .catch((err) => {
      console.error('[Seed Batch 5 CLI] Failed with error:', err);
      process.exit(1);
    });
}
