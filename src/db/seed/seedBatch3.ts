...
import { BATCH_3_ARTICLES } from '../../data/articles/batch3Articles';

export async function seedBatch3Articles() {
  console.log(`[Seed Batch 3] Ingesting ${BATCH_3_ARTICLES.length} US articles into Turso DB...`);
  const client = getTursoClient();

  for (const art of BATCH_3_ARTICLES) {
  ...
  console.log(`[Seed Batch 3] Successfully synced ${BATCH_3_ARTICLES.length} articles to Turso database!`);
}

if (process.argv[1] && process.argv[1].includes('seedBatch3')) {
  seedBatch3Articles()
    .then(() => {
      console.log('[Seed Batch 3 CLI] Done!');
      process.exit(0);
    })
    .catch((err) => {
      console.error('[Seed Batch 3 CLI] Failed with error:', err);
      process.exit(1);
    });
}
