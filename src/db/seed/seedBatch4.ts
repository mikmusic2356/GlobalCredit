...
import { BATCH_4_ARTICLES } from '../../data/articles/batch4Articles';

export async function seedBatch4Articles() {
  console.log(`[Seed Batch 4] Ingesting ${BATCH_4_ARTICLES.length} US News & Trends articles into Turso DB...`);
  const client = getTursoClient();

  for (const art of BATCH_4_ARTICLES) {
  ...
  console.log(`[Seed Batch 4] Successfully synced ${BATCH_4_ARTICLES.length} News & Trends articles to Turso database!`);
}

if (process.argv[1] && process.argv[1].includes('seedBatch4')) {
  seedBatch4Articles()
    .then(() => {
      console.log('[Seed Batch 4 CLI] Done!');
      process.exit(0);
    })
    .catch((err) => {
      console.error('[Seed Batch 4 CLI] Failed with error:', err);
      process.exit(1);
    });
}
